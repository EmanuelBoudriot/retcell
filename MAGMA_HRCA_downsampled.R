...
# Load CTD data
load("./ctd/hrca_snrna_subset_majorclass_5000/ctd_hrca_snrna_subset_majorclass_5000.rda")
...
for (i in 1:7) {
  ...
  MAGMA_results <- celltype_associations_pipeline(
    ...
    ctd = ctd,
    ctd_species = "human",
    ctd_name = paste0("ctd_hrca_snrna_subset_majorclass_5000.", magma_dirs$name[i]),
    force_new = TRUE,
    run_linear = TRUE,
    save_dir = NULL
  )
...
write.table(merged_results, "Merged_results_HRCA_snrna_subset_majorclass_5000.tsv", quote = F, row.names = F, sep = "\t")
